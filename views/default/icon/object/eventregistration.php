	...
	if ($event->canEdit()) {
		...
		if ($user_relationship == EVENT_MANAGER_RELATION_ATTENDING_PENDING) {
			...
			if ($event->hasEventSpotsLeft()) {
				...
				$result .= "<li>";
				$result .= elgg_view("output/url", array("href" => 'action/event_manager/attendees/move_to_waitinglist?guid=' . $event->getGUID() . '&user=' . $entity->getGUID(), "text" => elgg_echo('event_manager:event:menu:user_hover:move_to_waitinglist'), "is_action" => true));
				$result .= "</li>";
			}
		} elseif ($user_relationship == EVENT_MANAGER_RELATION_ATTENDING_WAITINGLIST) {
			$result .= "<li>";
			$result .= elgg_view("output/url", array("href" => 'action/event_manager/attendees/move_to_attendees?guid=' . $event->getGUID() . '&user=' . $entity->getGUID(), "text" => elgg_echo('event_manager:event:menu:user_hover:move_to_attendees'), "is_action" => true));
			$result .= "</li>";
		}
